...
仓库内置了配套 Codex skills，位于 [`skills/`](./skills)。MCP 提供可执行的素材工具；skills 负责告诉 agent 如何定义美术方向、规划 Cocos 素材包、选择工具、尽量把相关图片批量生成到 contact sheet 后裁剪、保留生成元数据，并在导入前检查输出质量。

包含的 skills：

- [`cocos-art-direction-director`](./skills/cocos-art-direction-director)：定义 art bible、prompt 语言、视觉一致性、可读性和 Cocos-ready 美术 QA。
- [`cocos-asset-pipeline-director`](./skills/cocos-asset-pipeline-director)：通过 `cocos_asset_forge` MCP 工具规划并验证 Cocos Creator 素材工作。
- [`cocos-asset-review-director`](./skills/cocos-asset-review-director)：在素材接入 gameplay 前进行设计、视觉、技术和运行时审核。
...
使用这些 skills 时，请同时在 MCP 客户端中配置本 server，并暴露为 `cocos_asset_forge`。
