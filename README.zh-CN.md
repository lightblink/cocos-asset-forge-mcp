...
仓库内置了一个配套 Codex skill：[`skills/cocos-asset-pipeline-director`](./skills/cocos-asset-pipeline-director)。MCP 提供可执行的素材工具；skill 负责告诉 agent 如何规划 Cocos 素材包、选择工具、写稳定 prompt、保留生成元数据，并在导入前检查输出质量。
...
使用这个 skill 时，请同时在 MCP 客户端中配置本 server，并暴露为 `cocos_asset_forge`。
